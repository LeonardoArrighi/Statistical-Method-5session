...
# We normalize the considered interval in order to make probabilities sum to 1. 
# The reason behind this decision is related to our prior belief about the upper
# bound of N and the subsequent interval within each the latter is considered to
# belong to. 
...
# Mean - to compute the posterior mean and standard deviation we weight the values 
# of *N* with the normalized probabilities over the considered interval.  

weighted_mean <- sum(dtf$x * dtf$y)
cat(weighted_mean)

# Standard deviation - as for the standard deviation we'll be using the weighted mean.
...
# P(N > 150|y_{1},...,y_{n}) - since we're using normalized probabilities, it is
# enough to sum those associated with values of N greater than 150.
...
(c) The Mantel–Haenzel test is valid only if the male-to-female odds ratio for admission is
