...
Suppose that you have $n$ independent measurements of times between major aircraft disasters, $t_i$, and believe that the probability density function for the $t_i$’s is of the form: $f(t) = ke ^{−\lambda t}, t \geq 0$ where $\lambda$ and $k$ are the same for all $i$. 
...
$$
we can procede computing algebric steps to link the normal p.d.f. to the given one.
$$
\begin{align}
f(t) &= \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{1}{2 \sigma^2} \left( t^2 - 2\mu t + \mu^2 \right)}\\
&= \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{(x-1)^2}{2 \sigma^2} (t^2)} \qquad (\mu = xt)\\
\end{align}
$$
Hence:
$$
\begin{cases}
\lambda = \frac{(x-1)^2}{2\sigma^2} \\
...
$$
\begin{aligned}
k &=\frac{1}{\sqrt{2\pi\frac{(x-1)^2}{2\lambda}}} \\
&= \sqrt{\frac{\lambda}{\pi(x-1)^2}}.
\end{aligned}
$$
...
\Rightarrow \lambda &= \sum_{i=1}^n \frac{1}{2 x_i^2} . 
\end{aligned}
$$
...
$$
LR(H_0)=2\left(l(\hat{\lambda})-l(\hat{\lambda_{H_0}})\right)
$$
